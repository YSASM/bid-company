    ...
    for i in list_:
        for j in on_off:
            if i[0]!=j['name']:
                continue
            if j['status']=="2":
                list_[list_.index(i)].append("2")
                break
            else:
                list_[list_.index(i)].append("1")
                break
    return list_
